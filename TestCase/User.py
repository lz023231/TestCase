...
class TestCase(unittest.TestCase):
    ...
    def test02(self):
        ...
        try:
            ...
            time.sleep(1)
            UserPage(self.driver).input_user(u"cs公司")
            time.sleep(1)
            ...
            assert company in u"cs公司"

        except AssertionError as error:
            print(u"断言异常:%s" % error)
        except NoSuchElementException as errors:
            print(u"定位元素异常(2):%s" % errors)
    ...
    def test03(self):
        ...
        try:
            ...
            HomePage(self.driver).switch_iframe()
            UserPage(self.driver).click_add_user()
            UserPage(self.driver).input_user(u"测试公司")

            UserPage(self.driver).click_Save_company()
            BasePage(self.driver).switch_home_iframe()
            prmept = UserPage(self.driver).prompt()

            assert prmept == u"此公司已存在！"

        except AssertionError as error:
            print(u"断言异常:%s" % error)
        except NoSuchElementException as errors:
            print(u"定位元素异常(3):%s" % errors)
    ...
    def test04(self):
        ...
        try:
            ...
            # 分三步输入联系人姓名
            UserPage(self.driver).input_Contacts_name(u"联系人")
            UserPage(self.driver).input_Contacts_name(username)
            UserPage(self.driver).input_Contacts_name(name)
            # 职位
            UserPage(self.driver).input_Contact_position(u"跳梁小丑")
            # 手机号
